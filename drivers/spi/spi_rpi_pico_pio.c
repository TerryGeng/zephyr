...
#if defined(CONFIG_SPI_RPI_PICO_PIO_DMA)
static uint32_t spi_pico_pio_dma_setup(const struct device *dev, bool dir);
				      ...
				      int status)
{
	...
	if (dma_dev != dev_cfg->dma_config.dev) {
		 ...
		 * so this is branch will never run in theory.
		 */
		return;
	}
...
#define DMA_INITIALIZER(inst)                                                           \
	{                                                                               \
		...
		.tx_channel = COND_CODE_1(DT_INST_DMAS_HAS_NAME(inst, tx),              \
				(DT_INST_DMAS_CELL_BY_NAME(inst, tx, channel)), ({0})), \
		.rx_channel = COND_CODE_1(DT_INST_DMAS_HAS_NAME(inst, rx),              \
				(DT_INST_DMAS_CELL_BY_NAME(inst, rx, channel)), ({0})), \
}

#define DMAS_ENABLED(inst) (DT_INST_DMAS_HAS_NAME(inst, tx) && DT_INST_DMAS_HAS_NAME(inst, rx))
...
#define SPI_PICO_PIO_INIT(inst)                                                                    \
	...
	static struct spi_pico_pio_config spi_pico_pio_config_##inst = {                           \
		...
		COND_CODE_1(CONFIG_SPI_RPI_PICO_PIO_DMA,                                           \
			(.dma_config = DMA_INITIALIZER(inst), .dma_enabled = DMAS_ENABLED(inst),), \
			(.dma_enabled = false,)                                                    \
		) };                                                                               \
